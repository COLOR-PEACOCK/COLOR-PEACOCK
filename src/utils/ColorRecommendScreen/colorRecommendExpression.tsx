...
// 유사색 조합: 메인 컬러 포함 3가지
export function getAnalogousColors(hsl: [number, number, number]): string[] {
	return [
...
// 3색 조화: 메인 컬러 포함 3가지
export function getTriadicColors(hsl: [number, number, number]): string[] {
	return [
		`#${convert.hsl.hex([hsl[0], hsl[1], hsl[2]])}`,
...
// 4색 조화: 메인 컬러 포함 4가지
export function getTetradicColors(hsl: [number, number, number]): string[] {
	return [
		`#${convert.hsl.hex([hsl[0], hsl[1], hsl[2]])}`,
...
// 밝게(Tint) : 메인컬러 포함 6가지
export function getTintColors(hsl: [number, number, number]): string[] {
	const basicLightnessPiece = 12;
	const lightnessDiff = 100 - hsl[2];
	const lightnessDivideHigh = Math.floor(lightnessDiff / 5);
...
export function getShadowColors(hsl: [number, number, number]): string[] {
	const basicLightnessPiece = 12;
	const lightnessDivideLow = Math.floor(hsl[2] / 5);
	const lightnessPiece = Math.min(lightnessDivideLow, basicLightnessPiece);
	return [
		`#${convert.hsl.hex([hsl[0], hsl[1], hsl[2]])}`,
		`#${convert.hsl.hex([hsl[0], hsl[1], hsl[2] - lightnessPiece])}`,
		`#${convert.hsl.hex([hsl[0], hsl[1], hsl[2] - lightnessPiece * 2])}`,
